...
void Sandbox::onInit()
{
    ...
    RES.add<TextureResource>("cubeMap", cube_map);
    RES.add<TextureResource>("irradiance_map", irradiance_map);
    RES.add<TextureResource>("prefilter_map", prefilter_map);
    ...
    auto brdf_lut             = RES.get_shared<TextureResource>("brdf_lut");
